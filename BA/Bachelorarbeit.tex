...
Der Knoten $1$ hat die Blätter $4,5,6$ und $7$. Daher ist $\text{t}_1 = 4$. Die weiteren Variablen $\text{t}_2$ und $\text{t}_3$ sind jeweils $2$ und $\text{t}_4$ bis $\text{t}_7$ hat keine Blätter und sind somit $0$.
...
Somit ergeben sich für (i,j) die Möglichkeiten $(1,1)$, $(0,2)$ und $(2,0)$, so dass $i+j = r+1$ gilt.\\
$\neg \text{b}_1^2 \vee \neg \text{b}_1^3$\\
$\neg \text{b}_0^2 \vee \neg \text{b}_2^3$\\
$\neg \text{b}_2^2 \vee \neg \text{b}_0^3$\\
...
Bei dieser Variante werden erneut Formeln von \cite[][]{knuth} zur Hilfe genommen. Bei der sequentiellen Variante berechnet der Counter Teilsummen und vergleicht die Summe der Teilsummen mit $r$. Anhand von \cite[][]{knuth} werden neue Variablen eingeführt. Dafür müssen $n$ und $r$ wieder bekannt sein, wie bei Bailleux und Boufkhad. Es werden $(n-r)*r$ neue Variablen der Form $\text{s}_j^k$ für $1 \leq j \leq n - r$ und $1 \leq k \leq r$ gebildet. \\
...
Variablen der Form $\neg \text{s}_j^0$ und $\neg \text{s}_j^{r+1}$ dürfen gestrichen werden. %warum?
