...
\section{Kodierungen}
In diesem Abschnitt werden die unterschiedlichen Kodierungen von Cardianlity Constraints beschrieben. Das Ziel ist es, dass die Cardinality Constraints über die Menge der Boolschen Variablen in eine Konjunktive Normalform (KNF) gebracht werden. %Innerhalb der KNF gibt es Einschränkungen, die verlangen, dass höchstens $r$ der Booleschen Variablen 1 sein können. Rein in die Grundlagen
...
\end{figure}
\ \\
Da Naiv bei $k=10$ einen \textit{out of memory} Fehler hatte, wird es auch hier nicht im Graphen (siehe Abbildung \ref{fig:queens}) dargestellt. Zusätzlich haben die Kodierungen NNaiv ($k=50$), NOddEven ($k=65$) und BDD ($k=60$) das Speicherlimit überschritten. Bei $k=70$ hat SinzPar am wenigsten Klauseln benötigt. Danach folgen SinzSeq, NOwn und Bailleux. 
...
\end{figure}
\ \\
Naiv verzeichnet wieder ein \textit{out of memory} bei $k=1,0$ und bricht daher im Graphen (siehe Abbildung \ref{fig:cat}) ab. Alle anderen Kodierungen konnten bis $k=3,0$ die Anzahl der Klauseln ausgeben. Folgende Reihenfolge lässt sich hier feststellen: NNaiv, NOddEven, NOwn, BDD, SinzSeq, Bailleux und SinzPar. Die Reihenfolge bleibt ab $k=1,6$ bestehen. Davor schwanken NNaiv, NOddEven und NOwn, sowie Bailleux, SinzSeq und SinzPar.
...
\end{figure}
\ \\
Da sich der H-Tree (siehe Abbildung \ref{fig:tree}) und die Cheshire Cat ähneln, gibt es dieselbe Reihenfolge für die Anzahl der Klauseln ab $k=20$. Auch die Schwankungen verhalten sich ähnlich wie bei der Cheshire Cat.
...
\begin{figure}[htbp]
\includegraphics[width=3.3cm]{legende.png}
\DTLplot{KnuthBailleux S klau,KnuthSinz S klau,NiklasseBDDs S klau,SinzParallel S klau,NOddEven S klau,NOwn S klau,NNaiv S klau,Naiv S klau}{x=k,y=logavgClauses,style=lines,xticgap=1000,minx=1000,maxx=5000,yticpoints={4,5,6,7,8},yticlabels={$10^4$,$10^5$,$10^6$,$10^7$,$10^8$},xlabel={k},ylabel={Klauseln},xticpoints={1000,2000,3000,4000,5000},xticlabels={1000,2000,3000,4000,5000},miny=4,maxy=8,width=8.7cm,linecolors={red,cyan,blue,orange,green,black,ForestGreen,magenta},lines={\pgfsetdash{}{0pt},\pgfsetdash{}{0pt},\pgfsetdash{}{0pt},\pgfsetdash{}{0pt},\pgfsetdash{}{0pt},\pgfsetdash{}{0pt},\pgfsetdash{}{0pt},\pgfsetdash{}{0pt}}}
\caption{Synthetische Testfälle mit der Anzahl der Klauseln}
\label{fig:syn}
\end{figure}
\ \\
Bei den synthetischen Testfällen (siehe Abbildung \ref{fig:syn}) zeigt sich eine andere Reihenfolge der Kodierung anhand der Anzahl der Klauseln. Hier ist die Reihenfolge NNaiv, SinzPar, BDD, NOddEven, Bailleux, NOwn und SinzSeq. Ab $k=3000$ steigt die Anzahl der Klauseln bei NOwn langsamer als bei den restlichen Kodierungen. Naiv kann auf Grund des \textit{out of memory} Fehlers nicht im Graphen verzeichnet werden.
